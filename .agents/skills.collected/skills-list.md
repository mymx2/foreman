...
| `jdbc-dsl` | 个人创建 | JDBC DSL 查询构建（用户自定义提供） |
...
| `self-iterating-system` | 个人创建 | 自迭代 Multi-Agent 系统设计：五层架构、双闭环、可证伪交付物与分层记忆（行业无关） |
| `agentic-game-dev` | 个人创建 | 小游戏研发实操：调研→GDD→美术/音频→编码→关卡工厂→宿主接入→验收三性 |
